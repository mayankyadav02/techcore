...
export function HeroVisual({ stats }: { stats: CatalogueStat[] }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-[24px] border border-white/15 bg-white/[0.08] shadow-[0_24px_80px_rgba(0,0,0,0.28)] backdrop-blur-xl">
      {/* Premium grid */}
      <div
        ...
        style={{
          backgroundImage:
            "linear-gradient(to right, rgb(0 200 120 / 0.12) 1px, transparent 1px), linear-gradient(to bottom, rgb(255 255 255 / 0.05) 1px, transparent 1px)",
          backgroundSize: "28px 28px",
        }}
      />

      <motion.div
        className="relative m-3 overflow-hidden rounded-[20px] border border-white/10 bg-navy-950/60 p-4 backdrop-blur-xl sm:m-5 sm:p-5"
      ...
      >
        {/* Header */}
        <div>
          <p className="text-[0.65rem] font-semibold uppercase tracking-[0.16em] text-brand-bright">
            TechCore Platform
          </p>
          ...
          <>
            ...
            <ul className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-3">
              {published.slice(0, 6).map((item) => (
                <li
                  key={item.label}
                  className="rounded-xl border border-white/10 bg-white/[0.045] px-3 py-3 transition-all duration-200 hover:border-brand/30 hover:bg-brand/[0.07]"
                >
            ...
            <div className="mt-4 rounded-xl border border-white/10 bg-navy-900/60 p-4">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="h-2 w-2 rounded-full bg-brand-bright shadow-[0_0_12px_rgba(0,240,106,0.8)]" />
              </div>
              ...
              >
                {published.slice(0, 7).map((item) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className="w-full rounded-t-md bg-gradient-to-t from-brand/60 via-brand to-brand-bright transition-all duration-300 group-hover:from-brand group-hover:to-white"
                        style={{ height: `${height}px` }}
                      />

                      <span className="mt-2 truncate text-center text-[0.55rem] text-white/35">
                        {item.label}
                      </span>
                    </div>
                  );
